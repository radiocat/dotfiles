...
### PowerShell プロファイルの手動適用（任意）

このリポジトリには、PowerShell での操作を効率化するためのカスタム設定（エイリアスや Gemini CLI の連携など）を含んだ `profile.ps1` が用意されています。
ご自身の環境ポリシーや好みに合わせて、以下の手順で手動で適用してください。

#### ① 実行ポリシー（Execution Policy）の変更

Windows のデフォルト設定では、セキュリティのためスクリプトの実行が制限されています。[cite_start]ローカルのプロファイルを読み込むためには、実行ポリシーを `RemoteSigned`（ローカルスクリプトは許可、インターネットからのダウンロードには署名を要求）に変更する必要があります。

管理者権限で PowerShell を起動し、以下を実行してください。

```powershell
Set-ExecutionPolicy RemoteSigned -Scope CurrentUser
```
※ [cite_start]「開発の利便性を確保しつつ、最低限のセキュリティの壁は残しておく」という観点から、この設定を推奨しています。

#### ② プロファイルのリンク作成

既存のプロファイルを、本リポジトリの `profile.ps1` に紐付けます。通常の PowerShell ターミナルで以下のコマンドを実行してください。

```powershell
# 既存のプロファイルが存在する場合はバックアップ
if (Test-Path $PROFILE) { Rename-Item $PROFILE "$PROFILE.bak" }

# dotfiles のプロファイルへシンボリックリンクを作成
New-Item -ItemType SymbolicLink -Path $PROFILE -Target "$HOME\dotfiles\windows\profile.ps1" -Force
```

※ 完全に上書きせず、ご自身で必要な部分だけを現在のプロファイルにコピー＆ペーストして運用していただいても構いません。

#### ③ プロファイルリンクの解除（元に戻す場合）

設定したシンボリックリンクを解除し、適用前の状態に戻したい場合は、以下のコマンドを実行してください。
...
# バックアップが存在する場合は復元
if (Test-Path "$PROFILE.bak") { Rename-Item "$PROFILE.bak" $PROFILE }
```
...
仮想環境 (`.venv`) 内に構築された Aider を利用し、ターミナル上で LLM と協調しながらソースコードを直接編集・コミットするワークフローを提供します。Git と高度に連携し、変更理由に基づいた適切なコミットメッセージを自動生成します。
